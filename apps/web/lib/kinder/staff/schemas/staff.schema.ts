...
export const CreateStaffEmployeeSchema = z
  .object({
    schoolId: z.string().uuid(),
  ...
  .refine(
    (data) => !data.grantSystemAccess || Boolean(data.email),
    {
      message: 'Email is required when granting system access',
      path: ['email'],
    },
  );

export const UpdateStaffEmployeeSchema = z
  .object({
